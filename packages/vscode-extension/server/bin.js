...
// ../mbel-core/dist/lexer.js
var MbelLexer = class _MbelLexer {
  input = "";
  pos = 0;
  line = 1;
  column = 1;
  ...
  static ARROW_OPERATORS = /* @__PURE__ */ new Map([
    ["files", "ARROW_FILES"],
    ["tests", "ARROW_TESTS"],
    ["docs", "ARROW_DOCS"],
  ...
  static SINGLE_CHAR_OPERATORS = /* @__PURE__ */ new Map([
    // Temporal
    [">", "TEMPORAL_PAST"],
    ["@", "TEMPORAL_PRESENT"],
    ["?", "TEMPORAL_FUTURE"],
  ...
  static BRACKETS = /* @__PURE__ */ new Map([
  ...
  scanToken() {
    ...
    const bracket = _MbelLexer.BRACKETS.get(char);
    if (bracket) {
    ...
    const singleOp = _MbelLexer.SINGLE_CHAR_OPERATORS.get(char);
    if (singleOp) {
      this.advance();
  ...
  scanBracketedContent(closeChar, type, start) {
    const openChar = this.advance();
  ...
  scanArrowOperator(start) {
    ...
    const tokenType = _MbelLexer.ARROW_OPERATORS.get(keyword);
    if (tokenType) {
      const value = "->" + keyword;
  ...
  static UNICODE_OPERATORS = /* @__PURE__ */ new Set([
    "\u2248",
    "\u2713",
    "\u2717",
    "\u26A1",
  ...
  isIdentifierStart(char) {
    if (_MbelLexer.UNICODE_OPERATORS.has(char)) {
      return false;
    }
    return char >= "a" && char <= "z" || char >= "A" && char <= "Z" || char === "_" || char.charCodeAt(0) > 127;
  }
...
// ../mbel-core/dist/parser.js
var MbelParser = class {
  lexer = new MbelLexer();
  tokens = [];
  current = 0;
  errors = [];
  ...
  parse(input) {
    ...
    this.current = 0;
    this.errors = lexerResult.errors.map((e) => ({
      message: e.message,
      position: e.position
    }));
    const statements = [];
  ...
  parseLinkDeclaration() {
    ...
    const linkType = linkTypeToken.type === "LINK_FEATURE" ? "feature" : "task";
    let name = "";
    if (this.check("STRUCT_METADATA")) {
      const metadataToken = this.advance();
      name = metadataToken.value.slice(1, -1);
    } else {
  ...
  parseFileRefList() {
    ...
    const content = listToken.value.slice(1, -1);
    const files = [];
    if (!content.trim()) {
      return files;
    }
  ...
  tokenToChainOperator(type) {
    switch (type) {
      case "RELATION_DEFINES":
        return "defines";
      case "RELATION_LEADS_TO":
        return "leads_to";
      case "RELATION_FROM":
        return "from";
      case "RELATION_MUTUAL":
        return "mutual";
      case "RELATION_AND":
        return "and";
...
var MbelAnalyzer = class {
  ...
  sourceText = "";
  diagnosticsWithContext = /* @__PURE__ */ new Map();
  constructor(options = {}) {
    this.options = {
  ...
  analyzeText(source) {
    ...
    for (const error of parseResult.errors) {
      const isDuplicate = diagnostics.some((d) => d.range.start.line === error.position.line && d.range.start.column === error.position.column);
      if (!isDuplicate) {
        const diag = this.errorToDiagnostic(error.message, error.position);
        diagnostics.push(diag);
        this.diagnosticsWithContext.set(diag, { source });
  ...
  // --- Private methods ---
  errorToDiagnostic(message, position) {
    let code = "UNEXPECTED_TOKEN";
  ...
  checkCamelCase(document) {
    const diagnostics = [];
    const underscorePattern = /_/;
    const checkIdentifier = (name, start, end) => {
  ...
  checkMissingVersion(document) {
    const hasVersion = document.statements.some((stmt) => stmt.type === "VersionStatement");
    if (!hasVersion && document.statements.length > 0) {
      return [{
        ...
        message: "Document is missing version statement (e.g., \xA7MBEL:5.0)",
        source: "mbel"
      }];
    }
    return [];
  }
  checkUnusedSections(document) {
    const diagnostics = [];
    const sections = [];
    let currentSectionHasContent = false;
    for (const stmt of document.statements) {
      if (stmt.type === "SectionDeclaration") {
        const lastSection2 = sections[sections.length - 1];
        if (lastSection2 && !currentSectionHasContent) {
          diagnostics.push({
            range: { start: lastSection2.start, end: lastSection2.end },
            severity: "warning",
            code: "UNUSED_SECTION",
            message: `Section "${lastSection2.name}" has no content`,
            source: "mbel"
          });
        }
        sections.push(stmt);
  ...
  checkDuplicateSections(document) {
    const diagnostics = [];
  ...
  checkDuplicateAttributes(document) {
    const diagnostics = [];
   ...
   */
  checkLinkDeclarations(document) {
    const diagnostics = [];
    const links = [];
    const linkNames = /* @__PURE__ */ new Map();
    const decisionNames = /* @__PURE__ */ new Set();
    for (const stmt of document.statements) {
    ...
    for (const link of links) {
      ...
      if (link.blueprint !== null && link.blueprint !== void 0) {
   ...
   */
  checkUnquotedBlueprint(link, diagnostics) {
   ...
   */
  checkEntryPointFromSource(diagnostics) {
   ...
   */
  checkUnknownMarkers(diagnostics) {
   ...
   */
  checkLineRangeFormat(diagnostics) {
   ...
   */
  checkCircularDependencies(links) {
    const diagnostics = [];
  ...
  createClosingBracketFix(diagnostic, bracket) {
    const edit = {
      range: {
  ...
  createVersionInsertFix(diagnostic) {
    const edit = {
      range: {
        start: { line: 1, column: 1, offset: 0 },
        end: { line: 1, column: 1, offset: 0 }
...
var MbelServer = class {
   ...
   */
  onDidOpenTextDocument(uri, version, content) {
    this.documents.set(uri, { uri, version, content });
  }
  /**
   * Handle document change
   */
  onDidChangeTextDocument(uri, version, content) {
    this.documents.set(uri, { uri, version, content });
  }
  /**
  ...
  getHover(uri, position) {
    ...
    if (twoChar === "::" || twoChar === "||") {
      const info2 = OPERATOR_INFO[twoChar];
      if (info2) {
        return {
          contents: {
            kind: import_node.MarkupKind.Markdown,
            value: `**${info2.label}** - ${info2.category}
